...
<body>
	...
	<section id="features">
		...
		<div class="section-header wow fadeIn" data-wow-duration="1s">
			<h3 class="section-title">Revenue Generated</h3>
			...
			<table class="table1 table-bordered">
				...
				<tbody>
					...
					<c:forEach items="${cu}" var="u">
						<tr>
							<td>${u.bookId}</td>
							<td onclick="window.location.href='user=${u.id}'" class="td1">${u.id}</td>
							<td>${u.roomId}</td>
							<td>${u.spaceName}</td>
							<td>${u.fromDate}</td>
							<td>${u.toDate}</td>
							<td style="text-align: right;">${u.cost}</td>
						</tr>

					</c:forEach>
					<tr>
						<th colspan="6" style="text-align: center;">Total Cost</th>
